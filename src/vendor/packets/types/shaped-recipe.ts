...
import { NetworkItemInstanceDescriptor2 } from "./NetworkItemInstanceDescriptor";
...
class ShapedRecipe extends DataType {
	 ...
	 */
	public readonly resultants: Array<NetworkItemInstanceDescriptor2>;
	...
	public constructor(
		...
		resultants: Array<NetworkItemInstanceDescriptor2>,
		uuid: string,
		tag: string,
		priority: number,
		symmetrical: boolean,
	...
	public static read(stream: BinaryStream): ShapedRecipe {
		...
		const resultants: Array<NetworkItemInstanceDescriptor2> = [];
		...
		for (let index = 0; index < resultantsCount; index++) {
			// Read the resultant
			const resultant = NetworkItemInstanceDescriptor2.read(stream);
	...
	public static write(stream: BinaryStream, value: ShapedRecipe): void {
		...
		for (const resultant of value.resultants) {
			// Write the resultant
			NetworkItemInstanceDescriptor2.write(stream, resultant);
		}
