...
import { ComplexInventoryTransaction, ContainerId, InventoryAction, InventoryContentPacket, InventorySlotPacket, InventorySource, InventorySourceType, InventoryTransaction, InventoryTransactionPacket, LegacyTransaction, NetworkItemStackDescriptor } from "@serenityjs/protocol";

class Inventory {
    private readonly client: Client;
    private readonly MAX_ITEMS = 36;
    public items: (NetworkItemStackDescriptor)[] = new Array(this.MAX_ITEMS).fill(new NetworkItemStackDescriptor(0));

    constructor(client: Client) {
        this.client = client;
        this.client.on("InventoryContentPacket", (packet: InventoryContentPacket) => {
    ...
    dropItem(slot: number, count = 1) { 
        const packet = new InventoryTransactionPacket();
            packet.legacy = new LegacyTransaction(
                -this.client.tick,
                [{ containerId: ContainerId.Inventory, changedSlots: [slot] }]
            )

            let newItem = new NetworkItemStackDescriptor(
            ...
            let newItem2 = new NetworkItemStackDescriptor(
                this.items[slot].network,
                count,
                this.items[slot].metadata,
                this.items[slot].stackNetId,
