...
const writeToLog = (message: string) => {
    ...
    fs.appendFileSync(logFile, `${new Date().toISOString()} - ${sanitizedMessage}\n`);
};
...
console.log = (...args) => {
    const message = args.map(arg => 
        typeof arg === 'object' ? util.inspect(arg, { colors: false }) : String(arg)
    ).join(' ');
...
process.on('uncaughtException', (error) => {
	writeToLog(`[UNCAUGHT EXCEPTION] ${sanitizeToLatin(error.message)}\n${sanitizeToLatin(error.stack || '')}`);
});

process.on('unhandledRejection', (reason, promise) => {
	writeToLog(`[UNHANDLED REJECTION] at: ${sanitizeToLatin(String(promise))}\nReason: ${sanitizeToLatin(String(reason))}`);
});
